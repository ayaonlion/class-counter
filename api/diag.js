...
// Vercel Edge Function: /api/diag
// 耗时诊断版，只调用智谱，记录每个阶段耗时
...
用户输入："""${text}"""`;
  ...
  try {
    ...
    return jsonResponse(200, {
      ...
      timings: {
        start,
        bodyRead_ms: bodyRead - start,
        promptBuilt_ms: promptBuilt - start,
        fetchStart_ms: fetchStart - start,
        fetchResponse_ms: fetchResponse - start,
        firstByte_ms: firstByteAt - start,
    ...
    return jsonResponse(500, {
      ...
      status,
      error: err.message,
      firstByte_ms: firstByteAt ? firstByteAt - start : null,
      lastByte_ms: lastByteAt ? lastByteAt - start : null,
